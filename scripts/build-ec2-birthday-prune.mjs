#!/usr/bin/env node
/**
 * Build EC2 one-shot prune script from tmp-birthday-delete-ids.json
 *   node scripts/build-ec2-birthday-prune.mjs
 */
import fs from "fs";

const idsPayload = JSON.parse(fs.readFileSync("tmp-birthday-delete-ids.json", "utf8"));
const ids = idsPayload.deleteIds || [];
if (!ids.length) throw new Error("deleteIds empty");
...
const DELETE_IDS = new Set(${JSON.stringify(ids)});
...
fs.writeFileSync("tmp-ec2-prune-birthday.mjs", out, "utf8");
console.log(
  JSON.stringify(
    { out: "tmp-ec2-prune-birthday.mjs", ids: ids.length, bytes: out.length },
    null,
    2
  )
);
