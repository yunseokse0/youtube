...
export default function YoutubePage() {
  ...
  useEffect(() => {
    const stop = startYoutubePolling(url, apiKey, () => {}, async ({word, author, message}: Parameters<OnForbidden>[0]) => {
      const text = `금칙어(${word}) - ${author}: ${message}`;
      const ev = { at: Date.now(), word, author, message };
      appendForbidEvent(word, author, message);
  ...
  }, [words, notificationSettings, notificationPermission, apiKey, url]);
  ...
  useEffect(() => {
    ...
    const stop = startChatPolling(liveChatId, apiKey, (msg) => {
  ...
  const connect = async () => {
    ...
    const { liveChatId: id, videoId } = await getLiveChatInfo(url.trim(), apiKey);
    console.log(`[YouTube Connect] 연결 결과 - LiveChatId: ${id}, VideoId: ${videoId}`);
    setLiveChatId(id);
    setYoutubeVideoUrl(url.trim());
  };
  const saveKey = () => {
  ...
  const triggerTestAlert = () => {
    ...
    appendForbidEvent(firstWord, "테스트봇", `이 메시지에 ${firstWord} 가 포함되었습니다.`);
    setEvents((prev)=>[ev, ...prev].slice(0,200));
