...
/** 수동 시그 판매 OBS — 청크·런타임 오류 시 방송 화면 안내 */
export default function SigSalesManualOverlayError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[overlay/sig-sales-manual]", error);
  }, [error]);
  ...
  return (
    <main className="flex min-h-[100dvh] w-full items-center justify-center bg-neutral-950 p-6 text-white">
      <div className="w-full max-w-md rounded-xl border border-yellow-400/35 bg-black/50 p-5 text-center">
        <h2 className="text-xl font-bold text-yellow-100">수동 시그 오버레이 오류</h2>
        ...
        <p className="mt-3 text-left text-[11px] leading-relaxed text-neutral-400">
          올바른 주소 예:{" "}
          <code className="text-neutral-200">/overlay/sig-sales-manual?u=계정ID&hideSigBoard=1</code>
        </p>
        <button
          type="button"
          onClick={reset}
