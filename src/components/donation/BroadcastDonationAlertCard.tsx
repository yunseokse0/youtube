...
export default function BroadcastDonationAlertCard({ alert, className = "" }: Props) {
  const amountLabel = alert.amount.toLocaleString("ko-KR");
  const targetLabel = donationAlertTargetLabel(alert.target);
  ...
  return (
    ...
    >
      ...
      <div className="rounded-2xl border border-white/40 bg-gradient-to-b from-rose-300 to-rose-500 px-4 py-3 text-center shadow-[0_6px_22px_rgba(0,0,0,0.4)]">
        <div className="text-[11px] font-bold tracking-wide text-white/95 sm:text-xs">기여도 점수</div>
        <div className="mt-0.5 text-3xl font-black tabular-nums leading-none text-white drop-shadow-[0_2px_3px_rgba(0,0,0,0.45)] sm:text-4xl">
          {alert.contributionPoints.toLocaleString("ko-KR")}
        </div>
      </div>
    </div>
  );
