...
async function withPoolConn<T>(
  ...
  if (isMysqlCircuitOpen()) {
    throw new ErrorEnvelope({
      code: DIN_INFRA_202,
      message: "MySQL circuit open (cooldown)",
      layer: "infra",
    });
  }
  ...
  const totalTimeout = new Promise<never>((_, reject) => {
    timerDestroy = setTimeout(() => {
      timedOut = true;
      void resetMysqlPool();
      reject(new ErrorEnvelope({
        code: DIN_SHELL_101,
        message: `MySQL withPoolConn total timeout after ${POOL_ACQUIRE_AND_QUERY_TOTAL_TIMEOUT_MS}ms — Pool forcibly reset (hang-prevention)`,
        layer: "infra",
      }));
  ...
  const realWork = (async () => {
    try {
      ...
      if (!p) {
        throw new ErrorEnvelope({
          code: DIN_INFRA_202,
          message: "MySQL pool unavailable",
          layer: "infra",
        });
      }
      const c = await p.getConnection();
      if (timedOut) { try { c.destroy(); } catch {}
        throw new ErrorEnvelope({
          code: DIN_SHELL_101,
          message: "MySQL connection acquired but total timeout already fired — aborted",
          layer: "infra",
        });
      }
...
async function connExecuteWithTimeout(
  ...
  try {
    ...
    if (timedOut) {
      throw new ErrorEnvelope({
        code: DIN_SHELL_101,
        message: `MySQL query timeout after ${MYSQL_QUERY_TIMEOUT_MS}ms`,
        layer: "infra",
      });
    }
    throw err;
