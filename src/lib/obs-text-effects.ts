...
export const OBS_TEXT_EFFECT_OPTIONS: ObsTextEffectOption[] = [
  ...
  { id: "shake", label: "흔들림", hint: "좌우 살랑" },
  { id: "glow", label: "빛남", hint: "글로우" },
  { id: "rainbow", label: "무지개", hint: "색상 회전" },
  { id: "wave", label: "파도", hint: "글자별 웨이브" },
  { id: "blink", label: "호흡", hint: "밝기 왕복" },
  { id: "float", label: "둥실", hint: "상하 부유" },
  { id: "neon", label: "네온", hint: "네온 글로우" },
  { id: "heartbeat", label: "두근", hint: "부드러운 박동" },
...
/** 파도 — 글자 간 위상차(주기의 9%씩, 루프 끊김 방지) */
export function obsTextEffectWaveCharDelaySec(charIndex: number, speed = 1): string {
  const durSec = Number.parseFloat(obsTextEffectDurationSec("wave", speed)) || 1.8;
  const stagger = durSec * 0.09;
  return `${(charIndex * stagger).toFixed(3)}s`;
}

export const OBS_TEXT_EFFECT_STYLES_CSS = `
/* 0%/100% 동일 + ease-in-out infinite — 루프 경계 끊김 최소화 */
...
@keyframes obs-tx-neon {
  0%, 100% {
    text-shadow: 0 0 4px #fff, 0 0 10px currentColor, 0 0 18px currentColor;
  }
  50% {
    text-shadow: 0 0 6px #fff, 0 0 18px currentColor, 0 0 30px currentColor;
  }
}
...
.obs-text-fx-gradient {
  ...
  animation: obs-tx-gradient var(--obs-tx-dur) ease-in-out infinite;
}
